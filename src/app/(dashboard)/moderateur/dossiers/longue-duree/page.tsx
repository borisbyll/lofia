...
const STATUT_LABEL: Record<string, string> = {
  ...
  locataire_interesse: 'Locataire intéressé',
  proprietaire_accepte:'Propriétaire OK',
...
export default async function DossiersLDPage() {
  const { data: dossiers } = await supabaseAdmin
    .from('dossiers_longue_duree')
    .select('id, reference, statut, date_visite, created_at, bien:biens(titre, ville), locataire:profiles!locataire_id(nom, phone)')
    .not('statut', 'in', '(expire,refuse,contrat_signe)')
  ...
  return (
    <div className="p-4 md:p-6 pb-nav max-w-3xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="page-title">Dossiers Location Longue Durée</h1>
      </div>
      ...
      {(dossiers ?? []).length === 0 ? (
        ...
        <div className="space-y-3">
          {(dossiers ?? []).map((d: any) => (
            <Link key={d.id} href={`/moderateur/dossiers/longue-duree/${d.id}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-brun-doux">{d.bien?.ville} · {d.locataire?.nom}</p>
