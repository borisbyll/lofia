...
import { VILLES_TOGO, TYPES_BIEN, PRIX_RANGES_LOCATION, PRIX_RANGES_NUIT, RAYON_OPTIONS } from '@/lib/constants'
import type { Bien } from '@/types/immobilier'

const TYPES_LOCATION = TYPES_BIEN.filter(t => (t.categorie as readonly string[]).includes('location'))
...
export default function LocationPage({ initialBiens = [] }: { initialBiens?: Bien[] }) {
  ...
  useEffect(() => { load() }, [duree, ville, typeBien, prixRange])

  const load = async () => {
    setLoading(true)
    let q = supabase.from('biens')
      .select('id,slug,titre,categorie,type_bien,type_location,prix,prix_type,ville,commune,quartier,photos,photo_principale,superficie,nb_pieces,nb_chambres,nb_salles_bain,vues,favoris_count,statut,is_featured,owner_id,latitude,longitude, proprietaire:profiles!owner_id(id,nom,avatar_url,identite_verifiee)')
      .eq('statut', 'publie').eq('categorie', 'location')
      .order('is_featured', { ascending: false })
      .order('publie_at', { ascending: false })

    if (duree !== 'all') q = q.eq('type_location', duree)
    if (ville)           q = q.eq('ville', ville)
    if (typeBien)        q = q.eq('type_bien', typeBien)
    if (prixRange) {
      const ranges = duree === 'courte_duree' ? PRIX_RANGES_NUIT : PRIX_RANGES_LOCATION
      const range = ranges.find(r => r.label === prixRange)
  ...
  const prixOptions = duree === 'courte_duree' ? PRIX_RANGES_NUIT : PRIX_RANGES_LOCATION

  return (
    <div className="section">
      <div className="wrap">
        <div className="page-header">
          <h1 className="page-title">Louer un bien</h1>
          <p className="page-subtitle">Courte ou longue durée — appartements, maisons, studios, bureaux…</p>
        </div>

        {/* Onglets durée */}
        <div className="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl w-full sm:w-fit">
          {([{key:'all',label:'Tous'},{key:'longue_duree',label:'Longue durée'},{key:'courte_duree',label:'Courte durée'}] as const).map(tab => (
            <button key={tab.key} onClick={() => setDuree(tab.key)}
              className={cn('flex-1 sm:flex-none px-3 sm:px-4 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition-all text-center', duree === tab.key ? 'bg-white shadow-sm text-primary-500' : 'text-gray-500 hover:text-gray-700')}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="relative flex-1">
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Titre, ville, quartier…" className="input-field pl-10" />
          </div>
          <button onClick={() => setShowFilters(v => !v)}
            className={cn('btn flex-shrink-0 gap-2', showFilters ? 'btn-primary' : 'btn-outline')}>
            <SlidersHorizontal size={16} />Filtres{hasFilters && <span className="w-2 h-2 rounded-full bg-accent" />}
          </button>
        </div>
        ...
        {showFilters && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4 shadow-sm">
            {[
              ...
              { label: 'Type de bien', value: typeBien, onChange: setTypeBien, options: TYPES_LOCATION.map(t => ({value:t.value,label:t.label})), placeholder: 'Tous les types' },
              { label: duree === 'courte_duree' ? 'Prix / nuit' : 'Loyer mensuel', value: prixRange, onChange: setPrixRange, options: prixOptions.map(r => ({value:r.label,label:r.label})), placeholder: 'Tous les prix' },
            ].map(f => (
              <div key={f.label}>
            ...
            {hasFilters && (
              <div className="sm:col-span-3 flex justify-end">
                <button onClick={() => { setVille(''); setTypeBien(''); setPrixRange('') }} className="btn btn-ghost text-sm text-red-500 hover:bg-red-50 gap-1.5">
                  <X size={14} /> Effacer
                </button>
              </div>
            )}
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500">{geo.active ? `Aucun bien dans un rayon de ${rayonLabel}.` : 'Modifiez vos critères.'}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
