...
interface Props { dossier: any; token: string }

export default function DecisionAcheteurClient({ dossier, token }: Props) {
  const [loading,  setLoading]  = useState(false)
  const [resultat, setResultat] = useState<'interesse' | 'non_interesse' | null>(
    dossier?.acheteur_interesse === true  ? 'interesse' :
    dossier?.acheteur_interesse === false ? 'non_interesse' : null
  )
    ...
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <Link href="/vente" className="btn-primary block mt-4 text-center">Voir d&apos;autres biens</Link>
        </div>
      </div>
    )
  }
  ...
  async function repondre(reponse: 'oui' | 'non') {
    setLoading(true)
    const r = await fetch(`/api/vente/decision-acheteur?token=${token}&reponse=${reponse}`)
    ...
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <h1 className="font-black text-brun-nuit text-xl mb-2">Excellent !</h1>
          <p className="text-brun-doux text-sm mb-6">Nous contactons le vendeur. Réponse sous 24h.</p>
    ...
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <Link href="/vente" className="btn-outline block text-center">Voir d&apos;autres biens</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-cream flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-8 max-w-md w-full space-y-6">
        ...
        <div className="text-center">
          <h1 className="font-black text-brun-nuit text-xl mb-1">Êtes-vous intéressé par ce bien ?</h1>
          <p className="text-brun-doux text-sm">{bien?.titre} · {bien?.ville}</p>
          {bien?.prix && <p className="prix text-lg mt-1">{formatPrix(bien.prix)}</p>}
        </div>
